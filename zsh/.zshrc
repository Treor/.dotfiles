...
function () {
  ...
  export PS1="%F{green}${SSH_TTY:+%n@%m}%f%B${SSH_TTY:+:}%b%F{blue}%B%1~%b%F{yellow}%B%(1j.*.)%(?..!)%b%f%B%F{red}${SUFFIX}%f%b "
#%F{red}%B$USER"@"$HOST%b%f%B":""%F{blue}%B%1d%b %F{yellow}%B%(1j.*.)%(?..!)%b%f %B${SUFFIX}%b "
#"%F{green}${SSH_TTY:+%n@%m}%f%B${SSH_TTY:+:}%b%F{blue}%B%1~%b%F{yellow}%B%(1j.*.)%(?..!)%b%f %B${SUFFIX}%b "
  if [[ -n "$TMUXING" ]]; then
