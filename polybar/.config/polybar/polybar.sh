...
MONITOR=$mainMonitor polybar -c /home/treor/.config/polybar/config.ini main &> /dev/null &
secondaryDisplays=$(echo $connectedDisplays | sed 's/ /\n/g' | tail -n +2)
if [ ! -z "$secondaryDisplays" ]; then
    for display in $secondaryDisplays; do
        MONITOR=$display polybar -c /home/treor/.config/polybar/config.ini secondary &> /dev/null &
